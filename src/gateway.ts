...
export class Gateway {
  ...
  static async create(config: AppConfig): Promise<Gateway> {
    const receiptKey = Deno.env.get("SOVEREIGNLOOP_RECEIPT_HMAC_KEY") ?? "";
    return new Gateway(
      config,
  ...
  async handle(request: Request): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname === "/readyz") {
      return json({
        status: "ready",
        providers: this.config.providers.map((provider) => provider.id),
      });
    }
  ...
  private async chat(request: Request): Promise<Response> {
    ...
    try {
      ...
      const requestedProvider = request.headers.get("x-sovereignloop-provider");
      ...
      if (policy.decision === "deny" || !policy.provider) {
        ...
        const receipt = await this.receipts.create({
          requestHash: await sha256(originalJson),
          decision: "deny",
          provider: null,
          model: chat.model,
          findings,
      ...
      return json(recomposed, 200, {
        "x-sovereignloop-receipt": receipt.id,
        "x-sovereignloop-decision": policy.decision,
      });
    } catch (error) {
...
function validateChat(value: unknown): string | null {
  ...
  if (body.tools !== undefined || body.tool_choice !== undefined) {
    return "Tool execution is disabled at this trust boundary.";
  }
  return null;
}
...
function problem(status: number, code: string, detail: string, receiptId?: string): Response {
  return json({
    type: `urn:sovereignloop:error:${code}`,
    title: code,
    status,
    detail,
    ...(receiptId ? { receiptId } : {}),
